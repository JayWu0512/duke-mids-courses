...
DEFAULT_GEN_TOP_SKILLS = os.getenv(
    "TINY_GEN_TOP_SKILLS", "1"
...
def normalize_lazy(lf: pl.LazyFrame) -> pl.LazyFrame:
    ...
    lf = lf.with_columns(
        [
            ...
            pl.coalesce(
                [
                    # try parse as tz-aware UTC then drop tz
                    pl.col("posted_raw")
    ...
    # 3) normalize skills as list[str]
    lf = lf.with_columns(
        [
...
def main() -> None:
    ...
    if "desc" in tiny_jobs.columns and tiny_jobs.height >= 2:
        long_text = (
            "lorem ipsum " * 500
        )  # long description to test downstream NLP/text limits
